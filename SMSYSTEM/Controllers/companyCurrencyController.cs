               ...
               // objcompanyCurrencybll.SelectAllCurrency();
                ...
                try
                {
                    if (id > 0)
                    {
                        ...
                        int.TryParse(dt.Rows[0]["currencyIdx"].ToString(), out currencyId);
                        objcompanyCurrencyproperty.currencyIdx = currencyId;
                        decimal.TryParse(dt.Rows[0]["exhangeRate"].ToString(), out exchangeRate);
                        objcompanyCurrencyproperty.exhangeRate = exchangeRate;
                        //ViewBag.CurrencyList = objcompanyCurrencyproperty.currencyIdx;
                    }
                    else
                    {
