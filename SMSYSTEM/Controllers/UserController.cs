    ...
    public class UserController : Controller
    {
        ...
        public ActionResult AddNewUser(int? id)
        {
            if (Session["LOGGEDIN"] != null)
            {
                ...
                objUserProperty.branchIdx = int.Parse(Session["BRANCHID"].ToString());//It will have the value of session branchIdx
                objUser = new User_BLL(objUserProperty);
               ...
               // objUserProperty.departmentIdx = 1;
                objUser = new User_BLL(objUserProperty);
                DataTable dttt = objUser.SelectDepartment();
                List<Departments_property> DepartmentList = new List<Departments_property>();
                foreach (DataRow dr in dttt.Rows)
                {
                    Departments_property objdepartment = new Departments_property();
                    objdepartment.Department = dr["Department"].ToString();
                    
                    objdepartment.idx = Convert.ToInt32(dr["idx"].ToString());
                    DepartmentList.Add(objdepartment);
                }
                ViewBag.DepartmentLists = DepartmentList;
                ...
                if (id != null && id != 0)
                {
                    ...
                    objUserProperty.departmentIdx = int.Parse(dt.Rows[0]["departmentIdx"].ToString());
